...
evals %>%
  summarise(means = mean(score), medians = median(score), min = min(score), max = max(score), sds = sd(score), q1 = quantile(score, probs = 0.25), q3 = quantile(score, probs = 0.75))
...
evals %>%
  ...
  geom_smooth(method = "lm",
              se = FALSE) +
  labs(title = "Scatter Plot of Average Professor Evaluation Score to Beauty Average Rating",
       x = "Beauty Average Rating",
       y = "Average Professor Evaluation Score")
```
...
For an average beauty rating of 0, the average evaluation rating is 3.88. This doesnt make sense in the data, as there are no beauty ratings of 0.
...
score_gender_fit %>% flextable()
```

The linear model is:
score = 4.09 = 0.14 * Male
A male professor, on average, scores 0.14 higher on evaluations than a female professor. On average, a female professor score 4.09 on the ratings.
...
score_rank_fit %>% flextable()
```

The linear model is:
score = 4.28 - 0.12x Tenure Track - 0.14x Tenured.

$$\widehat{score}_i = 4.28 - 0.12\times TenureTrack_i - 0.14\times Tenured_i$$

The average rating for a tenured professor is 4.14. The average rating for a teaching professor is 0.14 points higher than a tenured professor. The average rating for a tenure track professor is 0.02 points higher than a tenured professor.

The average rating for a teaching track professor is 4.28. The average rating for a tenure track professor is 0.12 points lower than a teaching track professor. The average rating for a tenured professor is 0.14 points lower than a teaching professor.
...
score_rank_relevel_fit %>% flextable()
``` 
